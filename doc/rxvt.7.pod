...
=begin table
	...
	B<< C<l> >>	Disable mouse coordinates in locale-specific encoding.

=end table

=item B<< C<Pm = 1006> >> (X11 XTerm SGR mouse mode) (Compile frills)

=begin table

	B<< C<h> >>	Enable xterm SGR mouse coordinate reporting.
	B<< C<l> >>	Disable xterm SGR mouse coordinate reporting.
...
=begin table
	...
	B<< C<l> >>	Disable urxvt mouse coordinate reporting.
...
When mouse reporting is enabled and none of the extended modes 1005,
1006, 1015 is active, urxvt sends the following sequence on a mouse
event:
...
If mode 1015 is active, urxvt instead sends the sequence
...
where the mouse reporting codes are provided as decimal parameters
instead of octets.

This mode should be enabled I<before> actually enabling mouse reporting,
for semi-obvious reasons.

The first three parameters are C<code>, C<x> and C<y>. C<code> is the numeric
...
If mode 1015 is not active but mode 1006 is, urxvt sends the following
sequences:

=over 4

=item B<< C<< ESC [ < <code>;<x>;<y> M >> >> button press and motion

=item B<< C<< ESC [ < <code>;<x>;<y> m >> >> button release

=back

C<code> is the numeric code as for the other modes, encoded as a
decimal number, with two modifications. The offset of 32 is not
included and the lower 2 bits encodes the button number also on button
release (instead of the value C<3>). The type of event (press or
release) is encoded in the final character of the sequence (M or m).

C<x> and C<y> are the coordinates (1|1 is the upper left corner, just
as with cursor positioning).

=head2 Mode 1005

If modes 1006 and 1015 are not active but mode 1005 is, urxvt sends
the sequence
